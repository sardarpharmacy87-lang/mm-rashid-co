...
export default async function CustomerDashboard() {
  ...
  const [{ data: profile }, { data: enquiries }, { data: quotations }] =
    await Promise.all([
      supabase
        .from("profiles")
        .select(
          "full_name, company_name, email, phone, country, city, role, created_at",
        )
        .eq("id", user.id)
        .single(),
      supabase
      ...
      supabase
        .from("quotations")
        .select(
          "id, enquiry_id, quotation_number, currency, subtotal, shipping, tax, discount, total, valid_until, status, sent_at",
        )
        .eq("customer_id", user.id)
  ...
  const quoteMap = new Map(
    (quotations ?? []).map((quote) => [quote.enquiry_id, quote]),
  );
  ...
  return (
    <main className="portal-main">
      <div className="portal-title-row">
        ...
        {profile?.role === "admin" ? (
          <Link className="portal-button" href="/admin">
            Open admin portal
          </Link>
        ) : (
                      ...
                      <td>
                        {quote ? (
                          <>
                            ...
                            <Link
                              className="text-link"
                              href={"/customer/quotations/" + quote.id}
                            >
                              View quotation &amp; payment
                            </Link>
                          </>
                        ) : (
                          "—"
      ...
      <section className="portal-section">
        <div className="portal-section-head">
          <h2>Explore MM Rashid &amp; Co.</h2>
        </div>
        <div className="customer-link-grid">
          <Link href="/products">
            <strong>Products</strong>
            <span>Browse the complete product catalogue →</span>
          </Link>
          <Link href="/catalogue">
            <strong>Catalogue</strong>
            <span>View the company catalogue →</span>
          </Link>
          <Link href="/#heritage">
            <strong>Our history</strong>
            <span>Read about the workshop heritage since 1922 →</span>
          </Link>
          <Link href="/#workshop">
            <strong>Workshop</strong>
            <span>See the stitching process and workshop videos →</span>
          </Link>
        </div>
      </section>
    </main>
